...
const aspek = {
	...
	getAspekById: async (req, res) => {
		model
			.getAspekById(req.params.id)
			.then((result) => {
				if (result.status == "ok") {
					success(res, result.data);
				} else {
	...
	updateAspek: async (req, res) => {
		model
			.updateAspek(req.body, req.params)
			.then((result) => {
				if (result.status == "ok") {
					success(res, result.msg);
				} else {

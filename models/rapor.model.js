...
const rapor = {
    getAllRapor: async () => {
        try {
            let res = await fetch(`${process.env.SUPABASE_URL}/motion_rapor?select=id_rapor, tahun, rapor_ke, hobi, kesimpulan_diri, kesimpulan_lain, motivasi, motion_user(nama,proker,motion_jabatan(jabatan, id_jabatan),motion_departemen(departemen,singkatan, id_departemen), nim), motion_kehadiran(id_kehadiran, kegiatan, tanggal, status_kehadiran, id_rapor), motion_detail_rapor(id_detail_rapor, nilai, transparansi, motion_aspek(aspek, indikator, penjelasan_indikator, id_aspek), id_rapor)`, {
                method: 'GET',
                headers: {
    ...
    getRaporByTurnNIMYear: async ({ turn, nim, year }) => {
        try {
            ...
            let res = await fetch(`${process.env.SUPABASE_URL}/motion_rapor?select=id_rapor, tahun, rapor_ke, hobi, kesimpulan_diri, kesimpulan_lain, motivasi, motion_user(nama,proker,motion_jabatan(jabatan, id_jabatan),motion_departemen(departemen,singkatan, id_departemen), nim), motion_kehadiran(id_kehadiran, kegiatan, tanggal, status_kehadiran, id_rapor), motion_detail_rapor(id_detail_rapor, nilai, transparansi, motion_aspek(aspek, indikator, penjelasan_indikator, id_aspek), id_rapor)&${params}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${process.env.SUPABASE_API_KEY}`,
